...
import * as alt3 from "alt-server";
...
var InitXSyncPed = ({
  ...
  return class XSyncPed extends Entity {
    ...
    constructor(pos, syncedMeta) {
      super(pedPool, pos, syncedMeta, {}, 0, 20, 10);
    }
  };
};
...
// src/ped/class.ts
import * as alt4 from "alt-server";
var _Ped = class {
  static getByID(id) {
    return _Ped.pedsById[id] ?? null;
  }
  xsyncInstance;
  _valid = true;
  constructor(model, pos) {
    ...
    _Ped.pedsById[this.xsyncInstance.id] = this;
  }
  get id() {
  ...
  get pos() {
    return new alt4.Vector3(this.xsyncInstance.pos);
  }
  set pos(value) {
  ...
  destroy() {
    if (!this._valid)
      return;
    this._valid = false;
    delete _Ped.pedsById[this.id];
    this.xsyncInstance.destroy();
  }
};
var Ped = _Ped;
__publicField(Ped, "pedsById", {});
export {
  Ped,
  XPedsSync
};
